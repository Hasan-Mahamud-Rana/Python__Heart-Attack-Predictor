...
os.chdir("F:/Conestoga/Course/BigdataPgmmg/PROJECT/INPUT")
...
os.chdir("F:/Conestoga/Course/BigdataPgmmg/PROJECT/OUTPUT")
...
Classification_Type = [' - Positive',' - Negative',' - Macro Average',' - Weighted Average']
...
for n in range(Number_of_models):
    ...
    Conf_Matrix_List[n].append(round(Accuracy_List[n]*100))              # add accuracy
...
Conf_Matrix_List_DF.columns =['True Positive','False Positive','False Negative','True Negative','Accuracy in %']
...
for n in range(Number_of_models):
        ...
        if n == 0:
            ...
            Compare_List_DF = Compare_List_DF.rename(index={Index_Num: Model_Names[n]+" Prediction"})
        else:
            Index_Num = Index_Num+1
            Compare_List_DF = Compare_List_DF.rename(index={Index_Num: Model_Names[n]+" Prediction"})
...
Heart_Condition_Predictor_DF.to_excel('Patient_Heart_Condition_Predictor.xlsx', sheet_name="Prediction", index=False)
...
with pandas.ExcelWriter('Heart_Attack_Analysis.xlsx', engine='xlsxwriter') as writer:
    ...
    Conf_Matrix_List_DF.to_excel(writer, sheet_name='Model Comparison', index=True,startcol=(List_Classification_DF2.shape[1] + 2))
