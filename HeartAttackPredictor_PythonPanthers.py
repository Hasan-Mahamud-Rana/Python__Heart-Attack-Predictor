...
#              3. SVC Classification                                                        #
#              4. GaussianNB Classification                                                 #
...
Training_ML_File_DF.index += 1
Training_ML_File_DF = Training_ML_File_DF.rename_axis("S.No")
...
Patient_Record_DF.index += 1
Patient_Record_DF = Patient_Record_DF.rename_axis("S.No")

# Change the working directory
os.chdir("D:/Education/Semester 1/21S - Sec2/PROG 8420 Programming for Big Data (Radha)/Projects/Predictive Analysis/Output_Files")
...
# This function uses 7 Machine learning models and determines the accuracy rate of the predictions
...
List_Classification_DF2 = List_Classification_DF2.rename_axis("Parameters")
List_Classification_DF2 = List_Classification_DF2.round(decimals=2)
...
for n in range(Number_of_models):
    ...
    Conf_Matrix_List[n].append(
        round(Accuracy_List[n]*100))              # add accuracy
...
Conf_Matrix_List_DF.columns = [
    'True Positive', 'False Positive', 'False Negative', 'True Negative', 'Accuracy in %']
Conf_Matrix_List_DF = Conf_Matrix_List_DF.rename_axis("Models")
...
for n in range(Number_of_models):
    ...
    if n == 0:
        Compare_List_DF = Compare_List_DF.rename(index={n: 'Actual Value'})
        Index_Num = Index_Num + 1
        Compare_List_DF = Compare_List_DF.rename(
            index={Index_Num: Model_Names[n]+" Prediction"})
    else:
        Index_Num = Index_Num+1
        Compare_List_DF = Compare_List_DF.rename(
            index={Index_Num: Model_Names[n]+" Prediction"})
...
Compare_List_DF.index += 1
Compare_List_DF = Compare_List_DF.rename_axis("S.No")
...
Heart_Condition_Predictor_DF.index += 1
Heart_Condition_Predictor_DF = Heart_Condition_Predictor_DF.rename_axis("S.No")


# Generate Output Excel Spreadsheet file with Prediction results
Heart_Condition_Predictor_DF.to_excel(
    'Patient_Heart_Condition_Predictor.xlsx', sheet_name="Prediction", index=False)
...
with pandas.ExcelWriter('Heart_Attack_Analysis.xlsx', engine='xlsxwriter') as writer:
    ...
    Conf_Matrix_List_DF.to_excel(writer, sheet_name='Model Comparison', index=True, startcol=(
        List_Classification_DF2.shape[1] + 2))
...
#--------------------------------------------------------------------------------------------------#
    ...
    hc2.options.title.text = 'Compare Accuracy and Confusion Matrix for All The Models'
...
#--------------------------------------------------------------------------------------------------#
    ...
    hc4.options.title.text = 'Compare Accuracy and Confusion Matrix for All The Models <br/> List Positive'

    List_Positive = List_Classification_DF2.iloc[::4, :]
    hc4.options.xAxis.categories = list(List_Positive.index)
    hc4.options.series = [{'name': v1, 'data': [v2 for v2 in List_Positive[v1]]}
                          for v1 in List_Positive.columns]

#--------------------------------------------------------------------------------------------------#

    d5 = jp.QDiv(classes="q-gutter-lg row justify-center", a=wp)
    hc5 = jp.HighCharts(a=d5, options=Compare_deff)
    hc5.options.title.text = 'Compare Accuracy and Confusion Matrix for All The Models <br/> List Negative'

    List_Negative = List_Classification_DF2.iloc[1:len(
        List_Classification_DF2):4]
    hc5.options.xAxis.categories = list(List_Negative.index)
    hc5.options.series = [{'name': v1, 'data': [v2 for v2 in List_Negative[v1]]}
                          for v1 in List_Negative.columns]

#--------------------------------------------------------------------------------------------------#

    d6 = jp.QDiv(classes="q-gutter-lg row justify-center", a=wp)
    hc6 = jp.HighCharts(a=d6, options=risk_deff)

    hc6.options.title.text = 'Heart Attack Actual Result'
    Heart_Attack_Prediction = Training_ML_File_DF.iloc[:, -1]
    Heart_Attack_Prediction_Percentage = Heart_Attack_Prediction.value_counts(
        normalize=True).mul(100)

    hc6.options.series[0].name = 'Heart Attack Result'
    hc6.options.series[0].data = [{
        'name': 'Positive',
        'y': round(Heart_Attack_Prediction_Percentage[1], 2),
    }, {
        'name': 'Negative',
        'y': round(Heart_Attack_Prediction_Percentage[0], 2)
    }]

#--------------------------------------------------------------------------------------------------#

    d7 = jp.QDiv(classes="q-gutter-lg row justify-center", a=wp)
    hc7 = jp.HighCharts(a=d7, options=risk_deff)

    hc7.options.title.text = 'Heart Attack Predicted Result with Highest Percentage'
    hc7.options.series[0].name = 'Heart Attack Prediction'

    Predicted_Heart_Attack = Conf_Matrix_List_DF.loc[Conf_Matrix_List_DF['Accuracy in %'].idxmax(
    )]

    Predicted_Heart_Attack = Predicted_Heart_Attack.iloc[:-1]

    hc7.options.series[0].data = [{'name': v1, 'y': v2}
                                  for v1, v2 in zip(Predicted_Heart_Attack.index, Predicted_Heart_Attack)]
